...
enum MoodInsights {
    ...
    private static func dominant(_ s: MoodStats, into result: inout [MoodInsight]) {
        ...
        case .great:
            (text, level) = ("Чаще всего вам прекрасно — \(count) отметок. Так бывает редко: стоит заметить, что за работа и какие условия это дают.", .good)
        case .flow:
        ...
        case .nervous:
            (text, level) = ("Чаще всего тревожно (\(count) отметок). Тревога — самое дорогое из всего списка: она не проходит за выходные так, как усталость.", .alarm)
        case .angry:
            (text, level) = ("Чаще всего вы злитесь (\(count) отметок). Злость обычно не про задачи, а про людей и порядки: посмотрите, с кем и с чем эти дни были связаны.", .alarm)
        case .homeSoon:
            (text, level) = ("Чаще всего вы ждёте конца дня (\(count) отметок). Это не усталость и не скука: время на работе хочется проматывать, а это самая честная мера того, насколько работа своя.", .attention)
        case .quit:
            (text, level) = ("Чаще всего — желание уволиться (\(count) отметок). Это не настроение дня, а вывод о работе целиком.", .alarm)
        }
    ...
    private static func timeOfDay(_ s: MoodStats, into result: inout [MoodInsight]) {
        ...
        // «Скорее бы домой» само по себе ни о чём не говорит: к шести вечера
        // так чувствует себя половина работающих людей. Значение имеет время —
        // ждать конца дня с утра и ждать его к вечеру это разные вещи.
        let home = s.lateShiftShare(.homeSoon)
        if home.total >= 3 {
            let early = home.total - home.late
            if Double(early) / Double(home.total) >= 0.6 {
                result.append(MoodInsight(level: .attention,
                                          text: "Конца дня вы ждёте с утра — \(early) из \(home.total) отметок «скорее бы домой» до середины смены. Ждать вечера, когда впереди ещё целый день, тяжелее всего: похоже, дело не в усталости к вечеру, а в самой работе."))
            } else {
                result.append(MoodInsight(level: .info,
                                          text: "«Скорее бы домой» приходит ближе к вечеру — \(home.late) из \(home.total) отметок во второй половине смены. Это обычный конец рабочего дня, а не признак беды."))
            }
        }
    ...
    private static func quitting(_ s: MoodStats, into result: inout [MoodInsight]) {
        ...
        if count >= 6 || (count >= 3 && count > previous * 2) {
            result.append(MoodInsight(level: .alarm,
                                      text: "«Хочу уволиться» — \(Fmt.times(count)) за период, в \(Fmt.days(days))\(previous > 0 ? ", в предыдущем периоде \(previous)" : ""). Столько раз это не настроение: стоит решать вопрос с работой, а не пережидать."))
        } else if count >= 3 {
            result.append(MoodInsight(level: .attention,
                                      text: "«Хочу уволиться» — \(Fmt.times(count)) за период. Пока это не серия, но и не случайность: посмотрите, что было в эти дни."))
        } else {
            result.append(MoodInsight(level: .info,
                                      text: "«Хочу уволиться» — \(Fmt.times(count)) за период. На таком фоне это, скорее, плохой день."))
        }
    }
